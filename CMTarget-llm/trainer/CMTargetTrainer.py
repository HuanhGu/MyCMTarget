...
class CMTargetTrainer():
    ...
    """
    def __init__(self, configs, dataloader):
    ...
    def model_train(self, model, train_dataloader, epoch_id):
        "输入是train_dataloader"
        ...
        for compound_batch, protein_batch, label_batch in tqdm(train_dataloader):        
    ...
    def model_evaluate(self, evl_model, test_dataloader):
        "输入是test_dataloader"
        ...
        with torch.no_grad():
            ...
            loop = tqdm(test_dataloader, total=total, smoothing=0, mininterval=1.0)
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            loss = self.model_train(self.model, train_dataloader, i)
            recall, precision, f1, accuracy, auc, y_true, y_score = self.model_evaluate(self.model, test_dataloader)
